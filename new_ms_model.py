...
def oligodendrocyte_growth(parameters, results):
    ...
    remissiont0 = parameters['remissiont0']
    ...
    # ### in model, get stress level at time from 'stress' list, use that stress level to determine death rate at time
    # then add those deaths to L_deaths and total_L_dead, subtract from total_L_alive, total_L_stressed
    ...
    stress_1 = oxidative_stress_equation(stresst0, stress_length, 0, stress_type, n)
    ...
    for t in tqdm(remission_range_1, desc='Running Oligodendrocyte Stress, Remission #1',
        ...
        for i in range(1, int(max(stress_range_1))):
            current_stress_level = remission_equation(stress_length + 1, remission_length, stress_1[100 - i],
                                                      remission_type, n)[t - 101]
            kD = death_rate(current_stress_level, parameters)
            deaths = kD * L_stressed[i]
            if deaths > 0:
                L_stressed[i] -= deaths
...
def neuron_growth(parameters, results):
    ...
    for t in tqdm(stress_range_1, desc='Running Neuron Stress, Relapse #1', total=len(stress_range_1),
        ...
        N_stressed[t] = stress_prob(total_N_stressed[t - 1], parameters) * total_N_unstressed[t - 1]
...
def protein_shedding(parameters, results):
    ...
    for t in tqdm(shedding_range, desc='Running Protein Shedding Analysis', total=(len(shedding_range)), mininterval=1):
        ...
        # normal shedding (uh)
        dumped_N_ec[t] = uh_ec * total_N_alive[t] * a
        dumped_L_ec[t] = uh_ec * total_L_alive[t]
        dumped_N_nec[t] = uh_nec * total_N_alive[t] * a
        dumped_L_nec[t] = uh_nec * total_L_alive[t]
